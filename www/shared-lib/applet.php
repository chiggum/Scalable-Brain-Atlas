...
class applet_class {
  ...
  function standardFormHtml($submitLabel='Submit',$target="iframe",$lastValues=array()) {
    if ($target == 'iframe') {
      ...
      $ans .= '<tr><td>';
      $ans .= '<input type="button" name="submit" value="'.$submitLabel.'" onclick="applet.submitForm(\'APPLET_FORM\',\'RESPONSE_HANDLER\',\'FEEDBACK\')"/>';
      ...
      $ans .= $this->mainForm->formAsTableRows($lastValues);
      $ans .= '<tr><td>';
      $ans .= '<input type="submit" name="submit" value="'.$submitLabel.'"/>';
      $ans .= '</td></tr></table></form>';
    }
    return $ans;
  }

  function uploadFormHtml($submitLabel='Submit',$target="iframe",$lastValues=array()) {
    if ($target == 'iframe') {
      $ans = '<form method="post" enctype="multipart/form-data" id="APPLET_FORM" accept-charset="UTF-8"><table>';
      $ans .= $this->mainForm->formAsTableRows($lastValues);
      $ans .= '<tr><td>';
      $ans .= '<input type="button" name="submit" value="'.$submitLabel.'" onclick="applet.submitForm(\'APPLET_FORM\',\'RESPONSE_HANDLER\',\'FEEDBACK\')"/>';
      $ans .= '</td></tr></table></form>';
      ...
      $ans = '<form method="post" enctype="multipart/form-data" id="APPLET_FORM" method="'.$this->submitMethod.'"><table>';
      $ans .= $this->mainForm->formAsTableRows($lastValues);
      $ans .= '<tr><td>';
      $ans .= '<input type="submit" name="submit" value="'.$submitLabel.'"/>';
      $ans .= '</td></tr></table></form>';
